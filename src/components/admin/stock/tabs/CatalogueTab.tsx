...
export function CatalogueTab({ products, units, onCreateProduct, onEditProduct, onOpenPricesModal }: CatalogueTabProps) {
        ...
        <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold">{products.reduce((acc, p) => acc + p.variants.length, 0)}</p>
            </div>
...
function ProductCardsView({ products, units, onProductEdit, onPricesModalOpen }: {
  products: IProduct[];
  units: IUnit[];
...
function ProductCard({ product, units, onEdit, onPricesModalOpen }: {
  product: IProduct;
  units: IUnit[];
  onEdit: () => void;
        ...
        <div className="mb-4">
          ...
          <div className="space-y-3 max-h-40 overflow-y-auto">
            {product.variants.slice(0, 3).map((variant) => (
              <div key={variant.id} className="flex items-center justify-between text-sm bg-gray-50 rounded p-3">
