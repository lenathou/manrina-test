...
export function PriceValidationTab({ products, onOpenPricesModal }: PriceValidationTabProps) {
  ...
  const priceMetrics = {
    ...
    variantsWithPrice: products.reduce((acc, p) => 
      acc + p.variants.filter(v => v.price && v.price > 0).length, 0
    ),
    variantsWithoutPrice: products.reduce((acc, p) => 
      acc + p.variants.filter(v => !v.price || v.price === 0).length, 0
    ),
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center space-x-4">
            <div>
              ...
              <select
                value={filterStatus}
                onChange={(e) => setFilterStatus(e.target.value as 'all' | 'no-price' | 'low-price' | 'high-vat')}
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <div className="text-center">
                              ...
                              <div className="flex flex-col space-y-1">
                                ...
                                {variant.price && variant.price >= 5 && (!variant.vatRate || variant.vatRate.taxRate <= 10) && (
