...
function ProductRowWithGlobalStock({
  ...
  const priceText = !vr || vr.min == null || vr.max == null
    ? '-'
    : (vr.min === vr.max
        ? (vr.min.toFixed(2) + ' €')
  ...
  return (
    <div key={variant.id} className="p-2 bg-gray-50 rounded">
      ...
      <span className="block text-xs text-gray-500">{isLoadingPrices ? 'Chargement...' : priceText}</span>
    </div>
  );
})}
        </div>
    ...
    <div className="space-y-8">
      ...
      <div className="p-6">
        <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
          ...
          <div>
            {(() => { const toolbarActions = [{ id: 'validation-stock', label: 'Validation stocks producteurs', onClick: () => router.push('/admin/stock/validation-stock') }, { id: 'page-produits', label: 'Aller a produits', onClick: () => router.push('/admin/produits') }]; return (<ActionDropdown placeholder="Actions" actions={toolbarActions} />); })()}
          </div>
        </div>
      </div>
          ...
          <ProductTable>
            <ProductTable.Header>
              <ProductTable.HeaderRow>
                ...
                <ProductTable.HeaderCell>Stock calculÃ©</ProductTable.HeaderCell>
