...
function AdminGrowersPage({}: { authenticatedAdmin: IAdminTokenPayload }) {
    ...
    const { growers, totalPages, isLoading, error } = useGrowers({
        page: currentPage,
        limit: itemsPerPage,
        search: searchTerm,
    });
        ...
        onError: (error: unknown) => {
            ...
            if (error instanceof Error && error.message) {
                if (error.message.includes('constraint') || error.message.includes('foreign key')) {
                    errorMessage =
                        'Impossible de supprimer ce producteur car il est lié à des commandes ou des produits existants.';
    ...
    const approveGrowerMutation = useMutation({
        ...
        onError: (error: unknown) => {
            const errorMessage =
                error instanceof Error ? error.message : 'Erreur lors de la mise à jour du statut du producteur';
            showError(errorMessage);
        },
    });
                    ...
                    <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md relative">
                        ...
                        {successMessage && (
                            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4 flex items-start gap-3">
                                ...
                                <div className="text-green-800 text-sm leading-relaxed">{successMessage}</div>
                            </div>
                        )}
                        {formError && (
                            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4 flex items-start gap-3">
                                ...
                                <div className="text-red-800 text-sm leading-relaxed">{formError}</div>
                            </div>
                        )}
                    </div>
                </div>
